...
class NumuneTakipApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(btn_frame, text="Numune Ekle", command=self.add_sample).grid(row=0, column=0)
        tk.Button(btn_frame, text="Numune Güncelle", command=self.update_sample).grid(row=0, column=1)
        tk.Button(btn_frame, text="Numune Sil", command=self.delete_sample).grid(row=0, column=2)
        tk.Button(btn_frame, text="PDF Yazdır", command=self.export_pdf).grid(row=0, column=3)
        tk.Button(btn_frame, text="Tümünü PDF Yazdır", command=self.export_all_to_pdf).grid(row=0, column=4)
        tk.Button(btn_frame, text="Tüm Numuneleri Listele", command=self.list_all_samples).grid(row=0, column=5)
...
=======
        ...
        if not selected_items:
            messagebox.showwarning("Uyarı", "PDF oluşturmak için en az bir numune seçin!")
            return
...
=======
        ...
        pdf.cell(200, 10, "Seçilen Numuneler Listesi", ln=True, align='C')
>>>>>>> upstream/main
        pdf.ln(10)
